...
def main():
    ...
    if not args.skip_celltypefracs_baseline:
        ...
        # run linear probe
        test_mean_acc, test_std_acc, test_mean_auc, test_std_auc, test_mean_f1, test_std_f1 = run_linear_probe_baselines(args, embs, targets, folds, results_dir=results_dir)
    ...
    if not args.skip_catfracsmeans_baseline:
        ...
        if celltypefracs_df is None:
            ...
            celltypefracs_df.columns = ['_'.join(col).strip() for col in celltypefracs_df.columns.values] # Flatten the multi-level columns
        ...
        cat_fracsmeans_df = celltypefracs_df.merge(celltype_means_df, right_index=True, left_index=True)
    ...
    if not args.skip_kmeans_baseline:
        ...
        # in this case there are different embs for each fold, bc emb is trained on the train set
        # choose K - in theory this is hparam to tune. for now, just use fixed K
        k = args.kmeans_k
        embs = {} # in this case, embeddings change with each fold, so store in a dict

        # for each fold, run kmeans on the train set, and project the test set
        for fold in np.unique(folds):

            print(f"Running kmeans for fold {fold}")
            train_idx = np.where(folds != fold)[0]

            # get train and test data
            train_data = adata[train_idx]

            # get train and test embs
            if args.latent_idx_tokeep is None:
                train_x = pd.DataFrame(train_data.obsm[args.adata_layer], index=train_data.obs.index)
                full_x = pd.DataFrame(adata.obsm[args.adata_layer], index=adata.obs.index)
            else:
                train_x = pd.DataFrame(train_data.obsm[args.adata_layer][:,args.latent_idx_tokeep], index=train_data.obs.index)
                full_x = pd.DataFrame(adata.obsm[args.adata_layer][:,args.latent_idx_tokeep], index=adata.obs.index)

            # run kmeans on the train set
            from sklearn.cluster import KMeans
            kmeans = KMeans(n_clusters=k, random_state=args.seed).fit(train_x)

            # get the cluster labels for the full dataset
            full_clusters = kmeans.predict(full_x)

            # patient embeddings are the concatenation of mean embeddings per cluster
            if args.latent_idx_tokeep is None:
                cluster_means_df = pd.DataFrame(adata.obsm[args.adata_layer], index=adata.obs.index)
            else:
                cluster_means_df = pd.DataFrame(adata.obsm[args.adata_layer][:,args.latent_idx_tokeep], index=adata.obs.index)
            cluster_means_df['cluster'] = full_clusters
            cluster_means_df[args.pid_col] = adata.obs[args.pid_col].values
            cluster_means_df = cluster_means_df.groupby([args.pid_col,'cluster']).mean() # calculate means
            cluster_means_df.columns = [str(col) for col in cluster_means_df.columns] # convert to strings
            cluster_means_df = cluster_means_df.fillna(0).reset_index() # cell types that dont exist in a given sample - fill 0 for mean expression
            cluster_means_df = cluster_means_df.pivot(index=args.pid_col, columns='cluster') # Pivot the dataframe
            #cluster_means_df.columns = ['_'.join(col).strip() for col in cluster_means_df.columns.values] # Flatten the multi-level columns

            # get patient embeddings in same patient order
            embs[fold] = cluster_means_df.loc[pt_adata[args.pid_col]].values 
