...
def main():
    ...
    if args.pretrained_ckpt is not None:
        ...
        if not args.load_cached_pretrain_dataset:
            ...
            autoreg_train_dataset = AutoregPatientDataset(folder=args.pretrain_data_dir, pretrain_embedding=args.pretrain_embedding, min_num_cells = args.min_num_cells, max_num_cells=args.max_num_cells,
                                                        ...
                                                        split_ids=[0, 1, 2, 3, 4, 5, 6, 7, 8], patient_csv=args.patient_csv, debug=args.debug, annots_dir=args.annots_dir, latent_idx_tokeep=args.latent_idx_tokeep,
                                                        target_libnorm=args.libnorm, target_lognorm=args.lognorm, sizefactor_maxfrac=args.libnorm_maxfrac)
            
            # get blacklisted genes for size factors for counts data normalization
            if False: #WIP
                print("Num blacklisted genes for size factors: ", autoreg_train_dataset.highlyexpressed_sizefactor_mask.sum())
                if args.encoder == 'pseudo_bulk':
                    encoder.set_sizefactor_info(autoreg_train_dataset.highlyexpressed_sizefactor_mask.to(device))
            ...
            autoreg_test_dataset = AutoregPatientDataset(folder=args.pretrain_data_dir, pretrain_embedding=args.pretrain_embedding, min_num_cells=args.min_num_cells, max_num_cells=args.max_num_cells,
                                                        num_anchor_cells=args.num_anchor_cells, num_target_cells=args.num_target_cells, preload=args.no_preload, split_col=args.pretrain_fold_col, 
                                                        split_ids=[9], patient_csv=args.patient_csv, debug=args.debug, annots_dir=args.annots_dir, latent_idx_tokeep=args.latent_idx_tokeep,
                                                        target_libnorm=args.libnorm, target_lognorm=args.lognorm)
        ...
        print("Running autoregressive pretraining")
        train_flow(args, autoreg_train_dataset, autoreg_test_dataset, encoder, denoise_model, checkpoint_dir, writer=writer)
        model_weights = {
            'encoder': encoder.cpu().state_dict(),
            'denoise': denoise_model.cpu().state_dict()
        }
    ...
    if not args.skip_linear_probe:    
        ...
        test_mean_acc, test_std_acc, test_mean_auc, test_std_auc, test_mean_f1, test_std_f1 = run_linear_probe(args, encoder, full_adata, results_dir=results_dir)
...
def run_finetuning(args, full_adata, encoder, sample_dim, checkpoint_dir, results_dir, device, writer, freeze_encoder=False):
    if not args.dont_run_kfold: #by default, run kfold
            ...
            for testfold in all_folds:
                ...
                if args.pretrained_ckpt is not None: #this seems redundant, also written above TO DO REBECCA RB
                    model_weights = torch.load(args.pretrained_ckpt, weights_only=True)
                else:
