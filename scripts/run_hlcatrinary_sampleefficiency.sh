#! /bin/bash

export CUDA_VISIBLE_DEVICES=0

# define tag for this string of experiments
tag="hlcatrinary_sampleefficiency"

# define finetuning task
h5ad_loc="/data/rna_rep_learning/hlca_sikkema2023/hlca_sikkema2023_covidIPFnormal_unbalanced_n369_cxgcensus20240701.h5ad"
fold_col="grouped_fold"
adata_layer="scvi_cxgcensus_20240701"
pid_col="sample"
target_col="disease"

# which latents to use
latent_idx_tokeep="2 6 14 20 24 27 28 31 34 35 36 37 43 46"

# results for pre-training with diffusion
pretrain='diffusion'

python trainer_sampleefficiency.py \
  --encoder abmil \
  --adata_layer ${adata_layer} \
  --h5ad_loc ${h5ad_loc} \
  --pid_col ${pid_col} \
  --target_col ${target_col} \
  --fold_col ${fold_col} \
  --pretrain ${pretrain} \
  --tag ${tag} \
  --lr 1e-3 \
  --classifier_num_hidden_layers 4 \
  --pretrain_epochs 400 \
  --latent_idx_tokeep ${latent_idx_tokeep} \
  --pretrained_ckpt '/data/rna_rep_learning/scset/outputs/checkpoints/spectacular-green-cat-122-abmil-diffusion-hlcatrinary_hvlatents/pretrained.pt' \
  --skip_frozen_finetuning \
  --skip_endtoend_finetuning \

python trainer_sampleefficiency.py \
  --encoder cell_transformer \
  --adata_layer ${adata_layer} \
  --h5ad_loc ${h5ad_loc} \
  --pid_col ${pid_col} \
  --target_col ${target_col} \
  --fold_col ${fold_col} \
  --pretrain ${pretrain} \
  --tag ${tag} \
  --num_blocks 2 \
  --num_heads 4 \
  --pretrain_epochs 400 \
  --latent_idx_tokeep ${latent_idx_tokeep} \
  --pretrained_ckpt '/data/rna_rep_learning/scset/outputs/checkpoints/incredible-silver-bird-499-cell_transformer-diffusion-hlcatrinary_hvlatents/pretrained.pt' \
  --skip_frozen_finetuning \
  --skip_endtoend_finetuning \

# compare abmil and cell-transformer with no pretraining
pretrain='none'

python trainer_sampleefficiency.py \
  --encoder abmil \
  --adata_layer ${adata_layer} \
  --h5ad_loc ${h5ad_loc} \
  --pid_col ${pid_col} \
  --target_col ${target_col} \
  --fold_col ${fold_col} \
  --pretrain ${pretrain} \
  --tag ${tag} \
  --lr 1e-3 \
  --classifier_num_hidden_layers 4 \
  --latent_idx_tokeep ${latent_idx_tokeep} \
  --skip_frozen_finetuning \
  --skip_endtoend_finetuning \

python trainer_sampleefficiency.py \
  --encoder cell_transformer \
  --adata_layer ${adata_layer} \
  --h5ad_loc ${h5ad_loc} \
  --pid_col ${pid_col} \
  --target_col ${target_col} \
  --fold_col ${fold_col} \
  --pretrain ${pretrain} \
  --tag ${tag} \
  --num_blocks 2 \
  --num_heads 4 \
  --latent_idx_tokeep ${latent_idx_tokeep} \
  --skip_frozen_finetuning \
  --skip_endtoend_finetuning \

# pseudobulk baseline (no pretraining needed)
python trainer_sampleefficiency.py \
  --encoder pseudo_bulk \
  --adata_layer ${adata_layer} \
  --h5ad_loc ${h5ad_loc} \
  --pid_col ${pid_col} \
  --target_col ${target_col} \
  --fold_col ${fold_col} \
  --pretrain ${pretrain} \
  --tag ${tag} \
  --pretrain_epochs 400 \
  --latent_idx_tokeep ${latent_idx_tokeep} \
  --skip_frozen_finetuning \
  --skip_endtoend_finetuning \

# run baselines on downstream task

python trainer_baseline_encoders_sampleefficiency.py \
  --adata_layer ${adata_layer} \
  --h5ad_loc ${h5ad_loc} \
  --pid_col ${pid_col} \
  --target_col ${target_col} \
  --fold_col ${fold_col} \
  --tag ${tag} \
  --latent_idx_tokeep ${latent_idx_tokeep} \
  
echo "Done"
exit 0  





