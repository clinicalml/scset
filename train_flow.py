...
from modules.denoising_models import conditional_flow_matching_loss


def train_flow(args, train_dataset, test_dataset, encoder, denoise_model, checkpoint_dir, writer=None):
    ...
    for epoch in range(args.pretrain_epochs):
        acc = train_epoch(args, encoder, denoise_model, optimizer, train_dl, epoch, device, writer=writer)
        test_loss = evaluate(args, encoder, denoise_model, test_dl, epoch, device, writer=writer)
...
def train_epoch(args, encoder, denoise_model, optimizer, dl, epoch, device, writer=None):
    ...
    for iteration, batch in enumerate(dl):
        ...
        x0_batch = torch.randn_like(target_cells) #random noise
        loss = conditional_flow_matching_loss(denoise_model, x0_batch, target_cells, tidx, cond=patient_emb)
        loss.backward()
...
def evaluate(args, encoder, denoise_model, dl, epoch, device, writer=None):
